...
/**
 * **Why this is not a `Dialog`.** A dialog is its own window, and the ground with
 * the grain on it is painted once inside [FlowPayTheme]'s own box. A full-screen
 * dialog therefore lands outside that box and has to paint its own background —
 * which is a flat near-black rectangle with no noise on it, filling the whole
 * screen, which is precisely the surface the grain exists for. It would have been
 * the one place in the app that banded.
 *
 * So the deck is a screen rather than an overlay: while it is open the app *is*
 * the deck, [FlowPayApp] composes this instead of the Scaffold, and the theme's
 * ground shows through because nothing here paints one. That also disposes of the
 * navigation bar and the action button for free, which a deck wants gone anyway.
 */
@Composable
